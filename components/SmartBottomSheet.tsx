...
export function SmartBottomSheet({
  ...
  const renderCarouselView = () => (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-center py-3 shrink-0">
        <div className={`w-12 h-1.5 rounded-full ${isDark ? 'bg-gray-500' : 'bg-slate-300'}`} />
      </div>
      ...
      >
        {locations.slice(0, 10).map((location, index) => (
          <motion.div
            ...
            className={`min-w-[85vw] snap-center backdrop-blur-xl border rounded-3xl overflow-hidden transition-all cursor-pointer ${isDark ? 'bg-white/10' : 'bg-white/90'} ${
          ...
          >
            ...
            {/* Info */}
            <div className="p-4">
              <h3 className={`text-lg font-bold mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>{location.name}</h3>
              <p className={`text-sm mb-3 ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{location.category} • {location.distance}</p>
  ...
  const renderListView = () => (
    <div className="h-full flex flex-col">
      ...
      <div 
        className={`sticky top-0 z-30 flex items-center justify-between px-4 pt-3 pb-3 shrink-0 border-b backdrop-blur-2xl ${isDark ? 'border-white/10 bg-[#0F172A]/95' : 'border-slate-200 bg-white/95'}`}
      >
        {/* Result Counter */}
        ...
        <div className="absolute left-1/2 -translate-x-1/2 -top-2">
          <div className={`w-12 h-1 rounded-full ${isDark ? 'bg-white/30' : 'bg-slate-300'}`} />
        </div>
        ...
        <div className="px-4 pt-4 pb-32 space-y-3">
          {locations.slice(0, 20).map((location) => (
            <motion.button
              key={location.id}
              onClick={() => onLocationClick(location)}
              className={`w-full backdrop-blur-xl border rounded-2xl p-4 transition-all text-left ${isDark ? 'bg-white/5 hover:bg-white/10 border-white/10' : 'bg-white hover:bg-slate-50 border-slate-200'}`}
            ...
            >
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className={`font-bold mb-1 truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{location.name}</h4>
                  
                  {/* Rating & Distance */}
                  <div className={`flex items-center gap-2 text-xs mb-2 ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>
                    <div className="flex items-center gap-1">
                      <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
                      <span className={`font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>{location.rating}</span>
                    </div>
                    <span>•</span>
  ...
  return (
    <>
      ...
      >
        <div className={`h-full flex flex-col pointer-events-auto backdrop-blur-2xl border-t rounded-t-3xl shadow-2xl ${isDark ? 'bg-[#0F172A]/95 border-white/10' : 'bg-white/95 border-slate-200'}`}>
      ...
      {isOpen && (
        <motion.button
          ...
          className={`fixed right-4 z-[60] backdrop-blur-xl p-2 rounded-full transition-all lg:hidden ${isDark ? 'bg-white/10 hover:bg-white/20' : 'bg-slate-900/10 hover:bg-slate-900/20'}`}
        >
          <X className={`w-5 h-5 ${isDark ? 'text-white' : 'text-slate-700'}`} />
        </motion.button>
      )}
    </>
  );
