  ...
  return (
    <>
      ...
      >
        ...
        <div 
          ref={containerRef}
          className={`pointer-events-auto ${isDark ? 'bg-[#0F172A] lg:bg-[#0F172A]/95' : 'bg-white lg:bg-white/95'} lg:backdrop-blur-xl rounded-t-3xl lg:rounded-3xl shadow-2xl overflow-hidden lg:max-w-2xl lg:w-full lg:max-h-[90vh] flex flex-col border-t lg:border ${isDark ? 'border-white/10' : 'border-slate-200'} relative ${
        ...
        >
          ...
          >
            ...
            <div className="relative aspect-[4/3] overflow-hidden">
              ...
              />
              <div className={`absolute inset-0 bg-gradient-to-t ${isDark ? 'from-[#0F172A]' : 'from-white'} via-transparent to-transparent`} />
            </div>
            ...
            <div className="px-6 py-4 space-y-3">
              ...
              <div className="flex items-center gap-3">
                <h2 className={`text-2xl font-bold flex-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>{place.name}</h2>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="flex items-center gap-1 text-yellow-400">
                  ...
                  <span className={`font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>{place.rating}</span>
                </div>
                <span className="text-gray-500">•</span>
                <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{place.reviewCount} đánh giá</span>
                <span className="text-gray-500">•</span>
                {/* Category - NỔI BẬT */}
                <span className={`px-3 py-1 rounded-full text-sm font-semibold border ${isDark ? 'bg-[#2DD4BF]/20 text-[#2DD4BF] border-[#2DD4BF]/30' : 'bg-teal-50 text-teal-600 border-teal-200'}`}>
                  {place.category}
                </span>
                {/* Saved Icon */}
                {isSaved && (
              ...
              {/* Địa chỉ */}
              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{place.address}</p>
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className={`${isDark ? 'text-gray-400' : 'text-slate-500'}`}>Giá trung bình:</span>
            ...
            {!isUnlocked && (
              <>
                ...
                <div className="px-6 py-4">
                  <h3 className={`text-lg font-bold mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>Thư viện ảnh</h3>
                  <div className="grid grid-cols-2 gap-2">
                    {thumbnailImages.map((img, index) => (
                      <div key={index} className="relative aspect-[4/3] rounded-xl overflow-hidden">
                        ...
                        <div className={`absolute inset-0 backdrop-blur-xl flex items-center justify-center ${isDark ? 'bg-white/10' : 'bg-slate-200/60'}`}>
                          <Lock className={`w-8 h-8 ${isDark ? 'text-white/60' : 'text-slate-400'}`} />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className={`px-6 py-4 ${isCarouselMode ? 'pb-[220px]' : 'pb-32'}`}>
                  <div className={`relative backdrop-blur-xl border rounded-3xl p-8 overflow-hidden min-h-[300px] ${isDark ? 'bg-white/5 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
                    ...
                    <div className="absolute top-8 right-8">
                      <div className={`w-16 h-16 backdrop-blur-md rounded-full flex items-center justify-center ${isDark ? 'bg-white/10' : 'bg-slate-200/80'}`}>
                        <Lock className={`w-8 h-8 ${isDark ? 'text-white/60' : 'text-slate-400'}`} />
                      </div>
                    </div>
                    ...
                    <div className="relative z-10 flex flex-col items-center justify-center py-16">
                      ...
                      {/* Micro-copy */}
                      <p className={`text-sm text-center mt-4 ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>
                        Summarized from {place.reviewCount}+ reviews
                      </p>
                      <p className={`text-xs text-center max-w-xs ${isDark ? 'text-gray-500' : 'text-slate-400'}`}>
                ...
                <div className={`px-6 py-4 ${isCarouselMode ? 'pb-[220px]' : 'pb-32'}`}>
                  ...
                  {place.aiTags && place.aiTags.length > 0 && (
                    ...
                    >
                      {place.aiTags.map((tag, index) => (
                        <span
                          key={index}
                          className={`px-4 py-2 text-sm font-semibold rounded-full border ${isDark ? 'bg-[#2DD4BF]/20 text-[#2DD4BF] border-[#2DD4BF]/30' : 'bg-teal-50 text-teal-600 border-teal-200'}`}
                        >
                          #{tag}
                        </span>
                      ))}
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <motion.div
                      ...
                      className={`backdrop-blur-xl border rounded-2xl p-5 ${isDark ? 'bg-teal-950/30 border-teal-500/30' : 'bg-teal-50 border-teal-200'}`}
                    >
                      <h4 className={`text-lg font-bold mb-4 flex items-center gap-2 ${isDark ? 'text-teal-400' : 'text-teal-600'}`}>
                      ...
                      <ul className="space-y-2">
                        {aiInsights.pros.map((pro, index) => (
                          ...
                          >
                            <CheckCircle className={`w-4 h-4 flex-shrink-0 mt-0.5 ${isDark ? 'text-teal-400' : 'text-teal-500'}`} />
                            <span className={`${isDark ? 'text-gray-300' : 'text-slate-600'}`}>{pro}</span>
                          </motion.li>
                        ))}
                      </ul>
                    </motion.div>
                    ...
                    <motion.div
                      ...
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: 0.2 }}
                      className={`backdrop-blur-xl border rounded-2xl p-5 ${isDark ? 'bg-orange-950/30 border-orange-500/30' : 'bg-orange-50 border-orange-200'}`}
                    >
                      <h4 className={`text-lg font-bold mb-4 flex items-center gap-2 ${isDark ? 'text-orange-400' : 'text-orange-600'}`}>
                      ...
                      <ul className="space-y-2">
                        {aiInsights.cons.map((con, index) => (
                          <motion.li
                            ...
                            className="flex items-start gap-2 text-sm"
                          >
                            <Shield className={`w-4 h-4 flex-shrink-0 mt-0.5 ${isDark ? 'text-orange-400' : 'text-orange-500'}`} />
                            <span className={`${isDark ? 'text-gray-300' : 'text-slate-600'}`}>{con}</span>
                          </motion.li>
                        ))}
                      </ul>
                    </motion.div>
