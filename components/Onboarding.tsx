...
export function Onboarding({ onComplete }: OnboardingProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col items-center justify-between overflow-hidden transition-colors duration-300 ${isDark ? 'bg-[#0B0E14]' : 'bg-slate-50'}`}>
        ...
        <div className="relative w-full max-h-[65vh] md:max-h-[600px] flex items-center justify-center">
          <AnimatePresence initial={false} custom={direction} mode="wait">
            ...
            >
              ...
              >
                {/* Glassmorphism Card */}
                <div className={`absolute inset-0 backdrop-blur-xl border rounded-2xl md:rounded-3xl shadow-2xl overflow-hidden ${isDark ? 'bg-white/5 border-white/10' : 'bg-white/60 border-slate-200'}`}>
                  <img
                ...
                {/* Glow Effect */}
                <div className={`absolute inset-0 rounded-2xl md:rounded-3xl blur-2xl -z-10 ${isDark ? 'bg-gradient-to-b from-[#8B5CF6]/20 via-transparent to-[#7C3AED]/20' : 'bg-gradient-to-b from-[#8B5CF6]/10 via-transparent to-[#7C3AED]/10'}`} />
              </motion.div>
              ...
              {isLastSlide && (
                <motion.button
                  ...
                  style={{ boxShadow: '0 0 40px rgba(255, 111, 76, 0.6)' }}
                >
                  B&#7855;t &#273;&#7847;u kh&#225;m ph&#225;
                </motion.button>
              )}
      ...
      <div className="w-full max-w-md px-4 md:px-6 pb-safe space-y-4 md:space-y-6 relative z-10 mb-6 md:mb-8">
            ...
            >
              <motion.div
                animate={{
                  ...
                  backgroundColor: currentSlide === index ? '#2DD4BF' : (isDark ? '#475569' : '#CBD5E1'),
                }}
        ...
        {!isLastSlide && (
          <button
            onClick={onComplete}
            className={`w-full py-2.5 md:py-3 font-semibold transition-colors ${isDark ? 'text-slate-400 hover:text-white' : 'text-slate-400 hover:text-slate-700'}`}
          >
            B&#7887; qua
          </button>
        )}
      ...
      {/* Decorative Elements */}
      <div className={`absolute top-10 right-10 w-3 h-3 rounded-full blur-sm animate-pulse ${isDark ? 'bg-white/20' : 'bg-slate-300/40'}`} />
      <div className={`absolute bottom-32 left-10 w-2 h-2 rounded-full blur-sm animate-pulse delay-1000 ${isDark ? 'bg-[#8B5CF6]/30' : 'bg-[#8B5CF6]/15'}`} />
      <div className={`absolute top-1/3 left-5 w-1 h-1 rounded-full animate-pulse delay-500 ${isDark ? 'bg-white/10' : 'bg-slate-300/30'}`} />
    </div>
  );
}
