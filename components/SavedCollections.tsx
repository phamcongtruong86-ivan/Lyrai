...
export function SavedCollections() {
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      {/* Header - Fixed */}
      <div 
        className="bg-slate-900/60 backdrop-blur-xl border-b border-slate-700/50 flex-shrink-0"
        style={{
          paddingTop: 'max(env(safe-area-inset-top), 16px)'
        }}
      >
        <h1 className="text-xl px-4 pt-4 pb-2 text-white">My Saved Places</h1>
      ...
      <div 
        ...
        style={{
          paddingBottom: 'max(calc(env(safe-area-inset-bottom) + 80px), 96px)' // Extra space for bottom nav
        }}
      >
        <Masonry columnsCount={2} gutter="16px">
          {filteredPlaces.map((place) => (
            <div
              key={place.id}
              className="bg-card border border-border rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all group"
            >
              {/* Image */}
              ...
              <div className="p-4 space-y-2">
                ...
                {/* AI Highlight Tags - Horizontal Scroll with Fade-out */}
