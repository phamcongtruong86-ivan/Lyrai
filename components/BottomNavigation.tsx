  ...
  return (
    <>
      ...
      <nav 
        ...
        style={{
          background: isDark ? 'rgba(15, 30, 50, 0.85)' : 'rgba(255, 255, 255, 0.85)',
          backdropFilter: 'blur(24px)',
          WebkitBackdropFilter: 'blur(24px)',
          borderTop: isDark ? '1px solid rgba(148, 163, 184, 0.1)' : '1px solid rgba(0, 0, 0, 0.08)',
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`
                ...
                ${activeTab === item.id ? 'text-primary' : isDark ? 'text-slate-500' : 'text-slate-400'}
              `}
            >
      ...
      <aside 
        ...
        style={{
          background: isDark ? 'rgba(15, 30, 50, 0.6)' : 'rgba(255, 255, 255, 0.7)',
          backdropFilter: 'blur(16px)',
          WebkitBackdropFilter: 'blur(16px)',
          borderRight: isDark ? '1px solid rgba(148, 163, 184, 0.1)' : '1px solid rgba(0, 0, 0, 0.08)'
        }}
      >
        {/* Logo */}
              ...
              >
                ...
                {activeTab === item.id && (
                  <div 
                    ...
                    style={{ boxShadow: '0 0 8px rgba(255, 111, 76, 0.6)' }}
                  />
                )}
                {/* "Trợ lý" luôn có dot */}
                {item.id === 'home' && activeTab !== 'home' && (
        ...
        {credits !== undefined && (
          <div className={`mb-4 px-2 py-1.5 rounded-lg ${
            isDark 
              ...
              : 'bg-white/60 border border-slate-200/50 shadow-sm'
          }`}>
            <div className="flex items-center gap-1">
              <span className="text-amber-400 text-xs">⚡</span>
              <span className={`text-xs font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>{credits}</span>
            </div>
          </div>
        )}
        ...
        <div className="mt-auto">
          <button
            onClick={onOpenProfile}
            className={`w-10 h-10 rounded-full p-[2px] overflow-hidden hover:border-primary/50 transition-colors ${
              isDark 
