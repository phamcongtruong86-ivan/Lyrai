...
export function MapView({ credits, setCredits }: MapViewProps) {
  ...
  return (
    <div className="relative w-full h-full bg-[#0F172A] overflow-hidden flex">
      {/* MAP CANVAS (Full Width) */}
      <div className="flex-1 relative">
        {/* Dark Night Mode Map Background */}
        ...
        <div className="hidden lg:block absolute top-6 left-6 z-20 w-96">
          <div className="backdrop-blur-2xl bg-white/10 border border-white/20 rounded-3xl p-6 shadow-2xl space-y-4">
            {/* Search Input */}
            <div className="relative backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-4 shadow-lg">
            ...
            <div className="space-y-2">
              <p className="text-xs text-gray-400 font-semibold">Khám phá nhanh</p>
              <div className="flex flex-wrap gap-2">
                {vibeChips.map((chip) => (
                  <button
                    key={chip.id}
                    onClick={handleChipClick}
                    className="flex items-center gap-2 px-4 py-2 backdrop-blur-xl bg-white/5 hover:bg-[#FF6F4C]/20 border border-white/10 hover:border-[#FF6F4C]/30 rounded-full text-sm text-white whitespace-nowrap transition-all"
                  >
                    <span>{chip.icon}</span>
                    {chip.label}
                  </button>
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto scrollbar-hide">
              {getAllPlaces().slice(0, 5).map((location) => (
                <button
                  ...
                  className="w-full backdrop-blur-xl bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl p-3 transition-all text-left group"
                >
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-white text-sm mb-1 truncate group-hover:text-[#7C3AED] transition-colors">
                        {location.name}
                      </h4>
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto scrollbar-hide">
            {vibeChips.map((chip) => (
              <button
                key={chip.id}
                onClick={handleChipClick}
                className="flex items-center gap-2 px-3 py-2 backdrop-blur-xl bg-white/10 border border-white/20 rounded-full text-sm text-white whitespace-nowrap"
              >
                <span>{chip.icon}</span>
                {chip.label}
              </button>
        ...
        <button
          ...
          className="absolute bottom-[120px] right-8 z-20 backdrop-blur-xl bg-white/10 hover:bg-white/20 border border-white/20 p-4 rounded-full transition-all shadow-lg lg:bottom-8"
        >
