...
export function PlaceCard({ place, onViewDetails, onSave, onCancel, variant = 'full' }: PlaceCardProps) {
  ...
  return (
    <div className={`backdrop-blur-lg rounded-xl overflow-hidden border shadow-lg hover:shadow-xl transition-shadow ${isDark ? 'bg-[#1E293B]/95 border-slate-700/50' : 'bg-white/95 border-slate-200'}`}>
      ...
      <div className="p-4 md:p-6 lg:p-8 space-y-3 md:space-y-4 lg:space-y-5">
        ...
        <div className="flex items-start justify-between gap-2 md:gap-4">
          <div className="flex-1">
            <h3 className={`text-lg md:text-2xl lg:text-3xl mb-1 md:mb-2 line-clamp-2 font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{place.name}</h3>
          ...
          <div className="flex items-center gap-1 md:gap-1.5 text-sm md:text-base lg:text-lg whitespace-nowrap">
            ...
            <span className={`text-xs md:text-sm ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>({place.reviewCount} đánh giá)</span>
          </div>
        </div>
        ...
        <div className="space-y-2 md:space-y-3 text-sm md:text-base">
          ...
          <div className="flex items-start gap-2 md:gap-3">
            <MapPin className={`w-4 h-4 md:w-5 md:h-5 mt-0.5 flex-shrink-0 ${isDark ? 'text-gray-400' : 'text-slate-400'}`} />
            <span className={`flex-1 ${isDark ? 'text-gray-300' : 'text-slate-600'}`}>{place.address}</span>
          </div>
          
          {/* Hours */}
          <div className="flex items-center gap-2 md:gap-3">
            <Clock className={`w-4 h-4 md:w-5 md:h-5 flex-shrink-0 ${isDark ? 'text-gray-400' : 'text-slate-400'}`} />
            <span className={`${isDark ? 'text-gray-300' : 'text-slate-600'}`}>{hours}</span>
          ...
          {/* Price Range */}
          <div className="flex items-center gap-2 md:gap-3">
            <DollarSign className={`w-4 h-4 md:w-5 md:h-5 flex-shrink-0 ${isDark ? 'text-gray-400' : 'text-slate-400'}`} />
            <span className={`${isDark ? 'text-gray-300' : 'text-slate-600'}`}>{priceRange}</span>
          </div>
        </div>
        ...
        <div className="flex gap-2 md:gap-3 pt-2 md:pt-4">
          {onViewDetails && (
            <button
              ...
              className={`flex-1 py-3 md:py-4 lg:py-5 border-2 border-[#FF6F4C] text-[#FF8C42] rounded-xl hover:bg-[#FF6F4C]/5 transition-colors font-semibold text-sm md:text-base ${isDark ? 'bg-slate-800/50' : 'bg-slate-50'}`}
            >
              Xem ảnh & Đánh giá
            </button>
          )}
