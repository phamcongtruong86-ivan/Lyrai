...
export function HomeScreen({ onSaveLocation, onSheetOpenChange, userAvatar, onOpenProfile, credits }: HomeScreenProps) {
  ...
  const scanResultPlace: Place = {
    ...
    reviewCount: 342,
    priceRange: '₫₫',
    priceRangeAmount: '40.000 - 80.000',
    distance: '0.5 km',
  ...
  return (
    <div className={`h-full flex flex-col relative overflow-hidden ${isDark ? 'bg-[#0B0E14]' : 'bg-slate-50'}`}>
        ...
        <div className="w-full max-w-full md:max-w-7xl mx-auto flex flex-col gap-5 md:gap-8 p-4 md:px-12 md:py-16 pb-[320px] md:pb-32">
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="p-3 md:p-16 flex flex-col items-center justify-center relative overflow-hidden min-h-[100px] md:min-h-[500px]">
                  ...
                  <div className="w-12 h-12 md:w-48 md:h-48 mb-1.5 md:mb-8 relative">
                    <div className="absolute inset-0 bg-primary/30 blur-3xl rounded-full" 
                         style={{ boxShadow: '0 0 80px -20px rgba(255, 111, 76, 0.5)' }}></div>
                    
                    <div className={`relative w-full h-full border border-primary/40 rounded-xl md:rounded-[2.5rem] flex items-center justify-center shadow-2xl ${
                  ...
                  {/* Hero Text */}
                  <h1 className={`text-base md:text-6xl lg:text-7xl font-bold mb-0.5 md:mb-4 text-center tracking-tight max-w-md md:max-w-none ${
                    isDark ? 'text-white' : 'text-slate-900'
                  }`}>
                    Bây giờ bạn chọn quán nào?
                  </h1>
                  <p className={`text-[10px] md:text-xl lg:text-2xl text-center font-medium ${
                ...
                <div className="flex flex-col md:flex-row gap-3 md:gap-6 mt-3 md:mt-6">
                  ...
                  <button 
                    ...
                    className={`flex-1 p-3 md:p-12 flex flex-col items-center justify-center gap-2 md:gap-6 group hover:border-primary/50 transition-all duration-300 w-full text-center min-h-[90px] md:min-h-[260px] rounded-[2rem] backdrop-blur-xl border ${
                      isDark 
                        ? 'bg-slate-900/40 border-slate-800/50' 
                        : 'bg-white/70 border-slate-200/80 shadow-lg shadow-slate-200/50'
                    }`}
                  >
                    ...
                    <div>
                      <h3 className={`text-sm md:text-3xl font-bold mb-0 md:mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>Soi ảnh/video</h3>
                      <p className={`text-[10px] md:text-lg ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Phân tích không gian quán</p>
                    </div>
                  </button>
                  ...
                  <button 
                    ...
                    className={`flex-1 p-3 md:p-12 flex flex-col items-center justify-center gap-2 md:gap-6 group hover:border-slate-500 transition-all duration-300 w-full text-center min-h-[90px] md:min-h-[260px] rounded-[2rem] backdrop-blur-xl border ${
                      isDark 
                        ? 'bg-slate-900/40 border-slate-800/50' 
                        : 'bg-white/70 border-slate-200/80 shadow-lg shadow-slate-200/50'
                    }`}
                  >
                    <div className={`shrink-0 w-10 h-10 md:w-24 md:h-24 rounded-lg md:rounded-3xl flex items-center justify-center transition-colors ${
                      isDark 
                        ? 'bg-slate-800/60 border border-slate-700 group-hover:bg-slate-700' 
                        : 'bg-slate-100 border border-slate-200 group-hover:bg-slate-200'
                    }`}>
                      <span className={`material-symbols-outlined text-xl md:text-5xl ${isDark ? 'text-white' : 'text-slate-700'}`}>radar</span>
                    </div>
                    
                    <div>
                      <h3 className={`text-sm md:text-3xl font-bold mb-0 md:mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>Tìm quanh đây</h3>
                      <p className={`text-[10px] md:text-lg ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Khám phá địa điểm gần bạn</p>
                    </div>
                  </button>
                </div>
              </motion.div>
            ...
            {analyzeState === 'file-picker' && (
              ...
              >
                <div className={`w-full h-48 object-cover rounded-xl mb-6 backdrop-blur-xl border flex items-center justify-center ${
                  isDark ? 'bg-slate-900/40 border-slate-800/50' : 'bg-white/70 border-slate-200'
                }`}>
                  <span className="material-symbols-outlined text-slate-400 text-[64px]">photo_camera</span>
                </div>
              ...
              >
                {uploadedImage && (
                  <img 
                    src={uploadedImage} 
                    alt="Uploaded" 
                    className={`w-full h-48 object-cover rounded-xl mb-6 border ${
                ...
                {/* Progress Bar */}
                <div className={`w-full rounded-full h-2 mb-4 overflow-hidden ${isDark ? 'bg-slate-800/50' : 'bg-slate-200'}`}>
                  <motion.div
      ...
      {analyzeState === 'idle' && (
        <div 
          className="fixed bottom-[80px] md:bottom-0 left-0 right-0 z-40 pt-24 md:pt-20"
          style={{
            background: isDark
              ? 'linear-gradient(to top, #0B0E14 0%, rgba(11,14,20,0.95) 60%, transparent 100%)'
              : 'linear-gradient(to top, #f8fafc 0%, rgba(248,250,252,0.95) 60%, transparent 100%)'
          }}
        >
          <div className="max-w-full md:max-w-7xl mx-auto px-4 md:px-12 pb-4 md:pb-12 flex flex-col gap-4 md:gap-6">
            ...
            <div className="flex flex-col gap-2.5 md:gap-4">
              <div className="flex items-center gap-2 md:gap-3 px-1">
                ...
                <span className={`text-[10px] md:text-sm font-bold uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Gợi ý chủ đề</span>
              </div>
              <div className="flex overflow-x-auto gap-2.5 md:gap-4 pb-2 hide-scrollbar">
                {suggestionChips.map((chip) => (
                  <button
                    ...
                    className={`flex-none px-4 md:px-6 py-2 md:py-3.5 border rounded-xl md:rounded-2xl text-xs md:text-base font-medium hover:border-primary/50 transition-all whitespace-nowrap backdrop-blur-md ${
                      isDark 
            ...
            <div className="relative group">
              <div className="absolute inset-y-0 left-4 md:left-8 flex items-center pointer-events-none">
                <span className={`material-symbols-outlined text-xl md:text-3xl ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>search</span>
              </div>
              <input 
                ref={inputRef}
                className={`w-full backdrop-blur-3xl rounded-full py-4 md:py-8 pl-12 md:pl-20 pr-14 md:pr-32 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all shadow-2xl text-sm md:text-2xl border ${
                  isDark
      ...
      {isDetailsSheetOpen && selectedPlace && (
        <PlaceDetailsSheet
          ...
          autoUnlock={selectedPlace.id === 'scan-result-1'}
        />
      )}
